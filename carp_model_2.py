...
class Lake:
    ...
    def __repr__(self):
        return str({"value" : self.value,
                        ...
                        "connections" : str(self.connection_dict)})
...
def create_lake_list(): #Turns lake_dict into a list, because lists can be iterated through without changing order each time
    ...
    return(lake_key_list, lake_list)
...
def create_transition_matrix():
    ...
    for lake in lake_list:
        ...
        if staying_probability < 0:
            print("WARNING: Staying probability of {} is negative".format(lake_key_list[own_index]))
...
def read_lakes(file_name, row_range, key_col, value_col, carrying_cap_col, population_col):
    output_dict = {}
    with open(file_name) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=",")
        row_number = 0
        for row in csv_reader:
            if row_number in row_range:
                output_dict[row[key_col]] = Lake(float(row[value_col]), float(row[carrying_cap_col]), float(row[population_col]), {})  
            row_number += 1
    return output_dict

def update_connections(file_name, row_range, node_1_col, node_2_col, probability_col, lake_dict): #updates each lake's connections dict from a csv file
    with open(file_name) as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=",")
        row_number = 0
        for row in csv_reader:
            if row_number in row_range:
                #print("abc")
                node_1_key = row[node_1_col]
                node_2_key = row[node_2_col]     
                probability = float(row[probability_col])

                #print(node_1_key, node_2_key, probability)

                lake_dict[node_1_key].connection_dict[node_2_key] = probability
                #print(lake_dict[node_1_key].connection_dict)

            row_number += 1
    """for entry in lake_dict.values():
        print(entry.connection_dict)"""
    return lake_dict
...
"""lake_dict = {"l1" : Lake(100, 1000, 5, {"l2" : .1}), 
                         "l2" : Lake(50, 10, 0, {"l1" : .2, "r3" : 2}),
                         "r3" : Lake(70, 360, 0, {"l2" : .5})}
connection_list = [(lake_dict["l1"], lake_dict["l2"], {"passing_probability": .1})]"""



#file reading tests below:
lake_dict = read_lakes("Asian Carp - Sheet3.csv", range(1,31), 2, 5, 7, 6)
lake_dict = update_connections("Asian Carp - Connections.csv", range(1,59), 0, 3, 9, lake_dict)
connection_list = []

lake_list_output = create_lake_list()
lake_key_list = lake_list_output[0]
lake_list = lake_list_output[1]
